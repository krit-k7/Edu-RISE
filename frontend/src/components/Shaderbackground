...
void main(){gl_Position=position;}`;
  ...
  useEffect(() => {
    ...
    let gl2Supported = true;
    try {
      if (!canvas.getContext("webgl2")) gl2Supported = false;
    } catch {
      gl2Supported = false;
    }
    if (!gl2Supported) return;
...
// Gold/obsidian palette matching EDURISE's seal-and-ledger theme
// (#C9A24E accent over a near-black obsidian base)
